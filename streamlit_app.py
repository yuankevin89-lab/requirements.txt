    ...
    if st.text_input("管理員密碼", type="password", key="stat_pwd") == "kevin198":
        if sheet:
            ...
            if len(all_raw) > 1:
                ...
                week_df = df[(df[headers[0]].dt.date >= last_monday) & (df[headers[0]].dt.date <= last_sunday)]
...
st.caption("© 2026 應安客服系統 - 2/16 智慧交班回歸最終版")
