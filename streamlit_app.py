...
with tab1:
    ...
    if st.session_state.edit_mode:
        st.warning(f"⚠️ 正在編輯第 {st.session_state.edit_row_idx} 列紀錄")
    ...
    with st.form("my_form", clear_on_submit=True):
        ...
        with col2:
            ...
            user_name = st.selectbox("填單人", options=STAFF_LIST, index=STAFF_LIST.index(u_val) if u_val in STAFF_LIST else 0)
    ...
    try:
        ...
        if len(data) > 1:
            rows = data[1:]
            search_query = st.text_input("🔍 關鍵字搜尋", placeholder="搜尋車號、場站、描述...")
            ...
            if display_list:
                ...
                for r_num, r_data in reversed(display_list):
                    with st.container():
                        ...
                        with c5:
                            if st.button("📝", key=f"edit_{r_num}"):
                        ...
                        with c6:
                            # 純勾選欄位，無連動意義
                            st.checkbox(" ", key=f"check_{r_num}")
