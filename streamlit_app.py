...
try:
    ...
    # 【提醒】請確保下方的名稱與你的 Google Sheets 檔名完全一致
...
if conn_success:
    with st.form("my_form", clear_on_submit=True):
        ...
        with col2:
            category = st.selectbox("案件類別", ["繳費機故障", "發票缺紙或卡紙", "無法找零", "其他"])
        ...
        if submit:
            ...
            if user_name and description:
                try:
                    # 按照順序組成清單寫入 Sheets：[時間, 姓名, 類別, 來電人, 電話, 車號, 描述]
                    row_to_add = [now, user_name, category, caller_name, caller_phone, car_number, description]
