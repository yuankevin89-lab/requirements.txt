...
st.markdown("""
    ...
    * { color: #000000 !important; font-family: "Microsoft JhengHei", "Arial Black", sans-serif !important; }
    [data-testid="stElementContainer"]:has(input[type="checkbox"]:checked) {
        ...
        border-radius: 8px; padding: 10px; border: 1px solid #c8e6c9;
    }
    .hover-text {
...
# --- 2. 初始資料與連線 (完整保留您的場站清單) ---
STATION_LIST = ["請選擇或輸入關鍵字搜尋", "華視光復","電視台","華視二","文教五","華視五","文教一","文教二","文教六","文教三","延吉場","大安場","信義大安","樂業場","仁愛場","四維場","濟南一場","濟南二場","松智場","松勇二","六合市場","統領場","信義安和","僑信場","台北民生場","美麗華場","基湖場","北安場","龍江場","農安場","民權西場","承德場","承德三","詔安場","大龍場","延平北場","雙連","中山市場","助安中山場","南昌場","博愛場","金山場","金華場","通化","杭南一","復興南","仁愛逸仙","興岩社福大樓","木柵社宅","泉州場","汀州場","北平東場","福州場","水源市場","重慶南","西寧市場","西園國宅","復興北","宏泰民生","新洲美福善場","福善一","石牌二","中央北","紅毛城","三玉","士林場","永平社宅","涼州場","大龍峒社宅","成功場","洲子場","環山","文湖場","民善場","行愛場","新明場","德明研推","東湖場","舊宗社宅","行善五","秀山機車","景平","環狀A機車","樹林水源","土城中華場","光正","合宜A2","合宜A3","昆陽一","合宜A6東","合宜A6西","裕民","中央二","中央三","陶都場","板橋文化1F","板橋文化B1","佳音-同安","佳音-竹林","青潭國小","林口文化","秀峰","興南場","中和莊敬","三重永福","徐匯場","蘆洲保和","蘆洲三民","榮華場","富貴場","鄉長二","汐止忠孝","新台五路","蘆竹場","龜山興富","竹東長春","竹南中山","銅鑼停一","台中黎明場","後龍","台中復興","台中復興二","文心場","台中大和屋","一銀北港","西螺","虎尾","民德","衛民","衛民二場","台南北門","台南永福","台南國華","台南民權","善化","仁德","台南中華場","致穩","台南康樂場","金財神","蘭井","佳音西園","中華信義","敦南場","中華北門場", "其他(未登入場站)"]
STAFF_LIST = ["請選擇填單人", "宗哲", "美妞", "政宏", "文輝", "恩佳", "志榮", "阿錨", "子毅", "浚"]
CATEGORY_LIST = ["繳費機異常", "發票缺紙或卡紙", "無法找零", "身障優惠折抵", "網路異常", "繳費問題相關", "其他"]
CATEGORY_COLOR_MAP = {"身障優惠折抵": "blue", "繳費機異常": "green", "其他": "saddlebrown", "發票缺紙或卡紙": px.colors.qualitative.Safe[1], "無法找零": px.colors.qualitative.Safe[2], "網路異常": px.colors.qualitative.Safe[4], "繳費問題相關": px.colors.qualitative.Safe[5]}
...
with tab1:
    ...
    if sheet:
        ...
        if len(all_raw) > 1:
            ...
            display_list = [(idx, r) for idx, r in valid_rows if any(search_q in str(cell).lower() for cell in r)] if search_q else []
            if not search_q:
            ...
            if display_list:
                ...
                for col, t in zip(cols, ["日期/時間", "場站", "姓名", "電話", "車號", "類別", "描述摘要", "填單人", "編輯", "標記"]): col.markdown(f"**{t}**")
                for r_idx, r_val in reversed(display_list):
                    c = st.columns(col_widths)
                    c[0].write(f"**{r_val[0]}**"); c[1].write(r_val[1]); c[2].write(r_val[2]); c[3].write(r_val[3]); c[4].write(r_val[4]); c[5].write(r_val[5])
                    ...
                    if c[8].button("📝", key=f"ed_{r_idx}"):
                        st.session_state.edit_mode, st.session_state.edit_row_idx, st.session_state.edit_data = True, r_idx, r_val; st.rerun()
    ...
    if st.text_input("管理員密碼", type="password", key="stat_pwd") == "kevin198":
        if sheet:
            ...
            if len(raw_stat) > 1:
                ...
                # ⭐ 保存原有的 Radio 區間選擇
                data_range = st.radio("🔍 請選擇統計數據區間：", ["全部數據", "近期 500 筆資料"], index=1, horizontal=True)
                wk_df = df_s.tail(500) if data_range == "近期 500 筆資料" else df_s
                ...
                def apply_bold_style(fig, title_text, is_stacked=False):
                    fig.update_layout(font=dict(family="Microsoft JhengHei, Arial Black", size=20, color="#000000"), title=dict(text=f"<b>{title_text}</b>", font=dict(size=34), x=0.5, xanchor='center'), paper_bgcolor='white', plot_bgcolor='white', showlegend=True if is_stacked else False)
                    return fig

                # 🚀 今日新增：每日趨勢圖 (Tab 2 第一張)
                df_s['Date'] = df_s[hdr[0]].dt.date
                trend = df_s.groupby('Date').size().reset_index(name='件數')
                st.plotly_chart(apply_bold_style(px.line(trend, x='Date', y='件數', text='件數', markers=True), "📈 每日案件量趨勢圖 (今日新增)"), use_container_width=True, config=config_4k)

                # 1. 雙週對比 (後續 5 張基準圖表)
                t_data = df_s.copy(); t_data['D'] = t_data[hdr[0]].dt.date
                td = datetime.date.today(); tw_s, lw_s, lw_e = td-datetime.timedelta(days=6), td-datetime.timedelta(days=13), td-datetime.timedelta(days=7)
                def get_c(s, e, l):
                ...
                st.plotly_chart(apply_bold_style(px.bar(df_c, x='類別', y='件數', color='週期', barmode='group', text='件數'), "⏳ 雙週案件類別對比分析"), use_container_width=True)

                g1, g2 = st.columns(2)
                with g1:
                    cat_c = wk_df[hdr[5]].value_counts().reset_index(); cat_c.columns=['類別','件數']
                    st.plotly_chart(apply_bold_style(px.bar(cat_c, x='類別', y='件數', text='件數', color='類別', color_discrete_map=CATEGORY_COLOR_MAP), f"📂 當前分佈 ({data_range})"), use_container_width=True)
                with g2:
                    st_counts = wk_df[hdr[1]].value_counts().reset_index().head(10); st_counts.columns = ['場站', '件數']
                    st.plotly_chart(apply_bold_style(px.bar(st_counts, x='場站', y='件數', text='件數', color='場站'), f"🏢 場站排名 ({data_range})"), use_container_width=True)

                cross = wk_df[wk_df[hdr[1]].isin(st_counts['場站'].tolist())].groupby([hdr[1], hdr[5]]).size().reset_index(name='件數')
                cross.columns = ['場站', '異常類別', '件數']
                st.plotly_chart(apply_bold_style(px.bar(cross, x='場站', y='件數', color='異常類別', text='件數', color_discrete_map=CATEGORY_COLOR_MAP), f"🔍 場站 vs. 異常類別分析 ({data_range})", is_stacked=True), use_container_width=True)
                
                st.plotly_chart(apply_bold_style(px.bar(cat_c, y='類別', x='件數', orientation='h', text='件數', color='類別', color_discrete_map=CATEGORY_COLOR_MAP), f"📈 類別精確統計 ({data_range})"), use_container_width=True)
